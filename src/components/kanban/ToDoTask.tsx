...
const ToDoTask = ({ toDoTasks, onStartDrag }: { toDoTasks: Task[], onStartDrag: any }) => {
  return (
    <div className="w-80 flex-shrink-0 bg-gray-100 rounded-lg p-3 flex flex-col shadow-sm" data-status="To Do">
      <div className="font-bold text-gray-700 mb-3 pb-2 border-b border-gray-200 flex justify-between items-center">
        <span>To Do</span>
        <span className="bg-gray-200 text-gray-600 text-xs py-1 px-2 rounded-full">{toDoTasks.length}</span>
      </div>
      <div className="flex-1 overflow-y-auto space-y-2 min-h-[150px] custom-scrollbar pb-10">
        {toDoTasks.length === 0 ? (
          ...
          toDoTasks.map(task => <TaskCard key={task.id} task={task} onStartDrag={onStartDrag} />)
        )}
      </div>
    </div>
  )
